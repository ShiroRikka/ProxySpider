...
class ProxyDB:
    ...
    def multiple_insert(self,iplist):
        ...
        iplist = [['192.168.1.1'],['10.0.0.2']]"""
        try:
            self.cursor.executemany(f'INSERT OR IGNORE INTO {self.table_name} (ip) VALUES (?)', iplist)
            self.conn.commit()
    ...
    def get_new_ips_from_db(self,limit: int = 10):
        ...
        """
        try:
            self.cursor.execute(f"SELECT ip FROM {self.table_name} WHERE  score >0 AND score < 70 ORDER BY score DESC LIMIT ?", (limit,))
    ...
    def get_best_ips_from_db(self,limit: int = None ):
        ...
        try:
            if limit is not None:
                self.cursor.execute(f"SELECT ip FROM {self.table_name} WHERE score >= 80 LIMIT ?",(limit,))
            else:
                self.cursor.execute(f"SELECT ip FROM {self.table_name} WHERE score >= 80")
            results = self.cursor.fetchall()
    ...
    def get_score(self,ip_port) ->int | None:
        ...
        """
        try:
            self.cursor.execute(f"SELECT score FROM {self.table_name} WHERE ip = ?",(ip_port,))
    ...
    def ip_count(self,num:str="> 0",num2:str="<= 30")->int:
        try:
            self.cursor.execute(f"SELECT COUNT(*) FROM {self.table_name} WHERE score {num} AND score {num2}")
    ...
    def create_index(self):
        try:
            self.cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_alive ON {self.table_name} (is_alive)')
            self.cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_last_checked ON {self.table_name} (last_checked)')
            self.cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_response_time ON {self.table_name} (response_time)')
            self.cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_score ON {self.table_name} (score)')
            self.cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_created_at ON {self.table_name} (created_at)')
            self.cursor.execute(f'CREATE INDEX IF NOT EXISTS idx_alive_created ON {self.table_name} (is_alive, created_at)')
            self.conn.commit()
            logger.info(f"数据库索引成功")
        except sqlite3.Error as e:
    ...
    def delete_old_ip(self):
        try:
            self.cursor.execute(f"DELETE FROM {self.table_name} WHERE created_at < datetime('now', 'localtime', '-1 day') AND score < 80" )
            self.conn.commit()
    ...
    def output_proxies_to_txt(self):
        try:
            self.cursor.execute(f"SELECT ip FROM {self.table_name} WHERE score = 100 ORDER BY response_time")
            results = self.cursor.fetchall()
